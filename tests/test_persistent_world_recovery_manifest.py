...
class PersistentWorldRecoveryManifestTests(unittest.TestCase):
    def _owners(self, minute: int = 41):
        return (
            ...
            signed_checkpoint(
                HOLDER_HISTORY_COVERAGE_BASELINE_CHECKPOINT_SCHEMA,
                minute,
                marker="holder-history-baselines-v1",
            ),
        )

    def _build(self, minute: int = 41):
        global_checkpoint, evidence_checkpoint, resource_checkpoint, holder_checkpoint, baseline_checkpoint = self._owners(minute)
        manifest = build_persistent_world_recovery_manifest(
            ...
            holder_history_coverage_baseline_checkpoint=baseline_checkpoint,
        )
        return (
            manifest,
            global_checkpoint,
    ...
    def test_v4_round_trip_selects_exact_five_owner_generation(self):
        manifest, global_checkpoint, evidence_checkpoint, resource_checkpoint, holder_checkpoint, baseline_checkpoint = self._build()
        ...
        self.assertEqual(
            restored.holder_history_coverage_baseline_checkpoint_sha256,
            baseline_checkpoint["sha256"],
        )

    def test_build_rejects_any_owner_from_different_semantic_minute(self):
        global_checkpoint, evidence_checkpoint, resource_checkpoint, holder_checkpoint, _ = self._owners()
        baseline_checkpoint = signed_checkpoint(
            HOLDER_HISTORY_COVERAGE_BASELINE_CHECKPOINT_SCHEMA,
            42,
            marker="holder-history-baselines-v1",
        )
    ...
    def test_reconcile_rejects_valid_but_wrong_baseline_generation(self):
        manifest, global_checkpoint, evidence_checkpoint, resource_checkpoint, holder_checkpoint, _ = self._build()
        other_baseline = signed_checkpoint(
            HOLDER_HISTORY_COVERAGE_BASELINE_CHECKPOINT_SCHEMA,
            41,
            marker="different-valid-baseline-generation",
        )
        with self.assertRaisesRegex(ValueError, "holder history coverage baseline checkpoint digest mismatch"):
            reconcile_persistent_world_recovery_manifest(
                ...
                world_resource_catalog_checkpoint=resource_checkpoint,
                resource_holder_transition_checkpoint=holder_checkpoint,
                holder_history_coverage_baseline_checkpoint=other_baseline,
            )
    ...
    def test_v4_reconcile_requires_baseline_checkpoint_candidate(self):
        manifest, global_checkpoint, evidence_checkpoint, resource_checkpoint, holder_checkpoint, _ = self._build()
    ...
    def test_v3_remains_readable_without_fabricating_baseline_owner(self):
        global_checkpoint, evidence_checkpoint, resource_checkpoint, holder_checkpoint, _ = self._owners()
        payload = {
            "schema": PERSISTENT_WORLD_RECOVERY_MANIFEST_V3_SCHEMA,
            "semantic_minute": 41,
            ...
            "resource_holder_transition_checkpoint_sha256": holder_checkpoint["sha256"],
        }
        ...
        self.assertEqual(restored.resource_holder_transition_checkpoint_sha256, holder_checkpoint["sha256"])
        self.assertIsNone(restored.holder_history_coverage_baseline_checkpoint_sha256)

    def test_v3_rejects_injected_baseline_candidate(self):
        global_checkpoint, evidence_checkpoint, resource_checkpoint, holder_checkpoint, baseline_checkpoint = self._owners()
        legacy = redigest(
            {
                "schema": PERSISTENT_WORLD_RECOVERY_MANIFEST_V3_SCHEMA,
                "semantic_minute": 41,
        ...
        with self.assertRaisesRegex(ValueError, "did not select holder history coverage baseline"):
            reconcile_persistent_world_recovery_manifest(
                legacy,
                global_npc_checkpoint=global_checkpoint,
                persistent_world_evidence_checkpoint=evidence_checkpoint,
    ...
    def test_legacy_v2_remains_readable_without_fabricating_newer_owners(self):
        global_checkpoint, evidence_checkpoint, resource_checkpoint, _, _ = self._owners()
        legacy = redigest(
            {
    ...
    def test_legacy_v1_remains_readable_without_fabricating_newer_owners(self):
        global_checkpoint, evidence_checkpoint, _, _, _ = self._owners()
        legacy = redigest(
            {
    ...
    def test_manifest_requires_current_baseline_checkpoint_schema(self):
        global_checkpoint, evidence_checkpoint, resource_checkpoint, holder_checkpoint, _ = self._owners()
        legacy_baseline = signed_checkpoint(
            "OUROS_HOLDER_HISTORY_COVERAGE_BASELINE_CHECKPOINT_V0",
            41,
            marker="legacy-baseline",
        )
        with self.assertRaisesRegex(ValueError, "requires schema"):
            build_persistent_world_recovery_manifest(
                ...
                world_resource_catalog_checkpoint=resource_checkpoint,
                resource_holder_transition_checkpoint=holder_checkpoint,
                holder_history_coverage_baseline_checkpoint=legacy_baseline,
            )
