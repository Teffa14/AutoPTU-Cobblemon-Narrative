...
class ResourceHolderTransitionCheckpointTests(unittest.TestCase):
    ...
    def test_snapshot_rejects_invalid_existing_ledger_instead_of_normalizing_it(self):
        invalid = ResourceHolderTransitionLedger(
            (
                ResourceHolderTransition(
                    transition_id="late",
                    resource_id="kit",
        ...
        snapshot = snapshot_resource_holder_transitions(invalid, semantic_minute=10)
        restored = restore_resource_holder_transitions(snapshot)
        self.assertEqual([item.transition_id for item in restored.ledger.transitions], ["early", "late"])
